...
2. **Search the KB for EACH technique**: `kb_search` (with source/family filters), `kb_pop_pattern`, `kb_get_cinematic_recipe`, `kb_get_vj_loop_reference`. Note which chunk grounds which part. **Also check the Palette** (`td_palette_list query=...`) before hand-building a common utility — movie playback, mapping, mixers, LUTs, UI panels and the user's own downloaded packs (RayTK, FunctionStore...) already exist as curated .tox; a Palette COMP beats a from-scratch rebuild in both reliability and user familiarity. Instantiate with `td_palette_load`.
   **Reproducing a specific tutorial?** `kb_get_tutorial video_id=...` (or `query=...` to find the id) returns EVERY chunk of the video, ordered — kb_search alone WILL miss segments, and one missing segment silently breaks a step-by-step rebuild. When a vision chunk and the raw transcript disagree on a value, **the transcript wins** — the vision pass misreads parameter panels (observed: "Alpha = 0.2, 2.9" fusing alpha and point size into one fake value). But for the LOOK, pixels win over both: Read 2-3 cached keyframes from `~/.cache/td-mcp/youtube/<channel>/<video_id>/keyframes/` (intro ≈ finished result, end = final composite) before the first judgment snapshot — the transcript never carries material rendering (emission, alpha/blending), camera framing, or perceived density (observed: all transcript values correct, render still nothing like the video).
...
## Scout + builder (optional, for LONG tutorial rebuilds)

For a long/complex tutorial rebuild, or when the main context is already heavy, spawn ONE read-only scout subagent at t0 to produce the build sheet (kb_get_tutorial → keyframes → kb_get_operator per class → ordered stages with exact internal param names + pitfalls) while the main loop scaffolds what doesn't depend on it (wrapper COMP, camera/lights/render/post chain, black-bg composite, checkpoint). Measured live (2026-07-13, two comparable Okamirufu tutorials): −15% wall-clock, and the decisive win is context — a ~1.2k-token sheet replaces tens of thousands of tokens of kb_get_operator dumps in the main window.

Rules:
- **Exactly one TD-mutating agent, ever.** The scout is strictly read-only: kb_* tools + Read on keyframes; every td_* tool is forbidden to it (two writers on one bridge = state races, broken checkpoints).
- The scout's sheet is trusted but not blindly: attribute-name case ('weight' vs 'Weight') and scene orientation are its observed failure modes — the visual loop catches both in one cycle each.
- Latency math: the scout takes ~5-6 min. If the parallelizable scaffold work is shorter, the builder idles — skip the scout for short tutorials, or give the builder more independent work (audio layer, palette loads, network hygiene).
...
- **Resolution silently clamped?** TD Non-Commercial caps ALL TOPs at 1280×1280: params read 1920×1080, `.width` says 1280, no error anywhere. Check `td.licenses.type` FIRST when a set resolution doesn't take — don't burn calls re-setting params or force-cooking. Separately, the project's global res multiplier scales TOPs down too: `resmult=False` per-op opts out. Diagnose by comparing `par.resolutionw.eval()` vs `.width`.
- OP-style params (selectPOP `pop`, feedbackTOP `top`, MAT assignments...) resolve relative paths from the node's PARENT COMP, not from the node itself — `../../source` written from inside a geo escapes one level too far and silently resolves to None (op renders empty, no error). Use absolute paths in OP params set via tools; a `select_src` showing 0 points with empty `inputs` is this bug.
- Sequential parameter blocks (linePOP `pt` points, mathcombinePOP `comb` operations...) CANNOT be grown from the bridge: `seq.X.numBlocks = n`, `par.X = n` and `insertBlock()` all fail ("Index invalid" / "name conflicts"). Work around structurally: linePOP default line runs (0,0,0)→(1,1,0) — correct with a transformPOP; a 2-operation mathcombine becomes two chained mathcombinePOPs. Confirmed live twice on TD 2025.32820.
- lumablurTOP widens the BRIGHT areas — on a dark scene that's the subject itself, so wiring it inline blurs your content (reads as "resolution flouuu"). For glow, keep the sharp pass and ADD an attenuated blurred copy (sharp → addTOP[0], lumablur → levelTOP opacity~0.45 → addTOP[1]); its 2nd input is a per-pixel width map that behaves unpredictably at non-tutorial resolutions — drop it unless the reference proves otherwise.
